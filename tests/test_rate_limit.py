...
@pytest.mark.skip(
    reason="Rate limiting with TestClient requires real time delays and may be flaky"
)
def test_predict_rate_limit_exceeded(client):
    ...
    responses = []
    for i in range(105):
        response = client.post("/predict", json={"text": f"Test rate limit {i}"})
...
def test_batch_rate_limit_exceeded(client):
    ...
    for i in range(25):
        response = client.post(
            "/batch", json={"texts": [f"Batch test {i}-1", f"Batch test {i}-2"]}
        )
        responses.append(response.status_code)
...
def test_rate_limit_headers_present(client):
    ...
    response = client.post("/predict", json={"text": "Test rate limit headers"})
...
def test_analytics_endpoints_rate_limited(client):
    ...
    for endpoint in endpoints:
        ...
        assert response.status_code in [
            ...
            429,
            500,
        ]  # 500 if DB not available in test
...
def test_ab_endpoint_rate_limited(client):
    ...
    response = client.post("/predict/ab", json={"text": "Test A/B rate limit"})
...
def test_rate_limit_different_endpoints_separate_limits(client):
    ...
    predict_responses = []
    for i in range(5):
        response = client.post("/predict", json={"text": f"Test {i}"})
    ...
    assert all(
        code != 429 for code in analytics_responses[:3]
