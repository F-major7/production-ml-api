...
def test_predict_ab_endpoint(client):
    ...
    response = client.post(
        "/predict/ab", json={"text": "This is a test for A/B testing"}
    )
...
def test_predict_ab_multiple_requests():
    ...
    for _ in range(20):
        response = client.post("/predict/ab", json={"text": "Test message"})
        data = response.json()
...
def test_ab_comparison_endpoint_structure(client_with_db):
    ...
    for i in range(10):
        client_with_db.post("/predict/ab", json={"text": f"Test message {i}"})
