...
@pytest.mark.skip(reason="Caching tests require manual verification with running Redis - see manual test section below")
...
def test_batch_predict_with_caching(client):
    ...
    for i in range(3):
        assert data2["predictions"][i]["sentiment"] == data1["predictions"][i]["sentiment"]
        assert data2["predictions"][i]["confidence"] == data1["predictions"][i]["confidence"]
...
def test_batch_predict_mixed_cache_hits_misses(client):
    ...
    response = client.post(
        "/batch",
        json={"texts": ["Already cached", "New text 1", "New text 2"]}
    )
