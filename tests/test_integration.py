...
def test_full_prediction_flow(client_with_db):
    ...
    # First request
    response = client_with_db.post("/predict", json={"text": unique_text})
    ...
    response2 = client_with_db.post("/predict", json={"text": unique_text})
...
def test_metrics_endpoint_increments(client):
    ...
    # Make a prediction
    response = client.post("/predict", json={"text": "Metrics test"})
    assert response.status_code == 200
...
def test_rate_limiting_triggers_429(client):
    ...
    for i in range(105):  # Limit is 100/minute
        response = client.post("/predict", json={"text": f"Rate limit test {i}"})
...
def test_batch_prediction_with_cache(client):
    ...
    response1 = client.post("/batch", json={"texts": texts})
    ...
    response2 = client.post("/batch", json={"texts": texts})
    assert response2.status_code == 200
    data2 = response2.json()
...
def test_ab_testing_integration(client):
    ...
    response = client.post("/predict/ab", json={"text": "A/B test integration"})
    assert response.status_code == 200
    data = response.json()
...
def test_error_handling_invalid_input(client):
    ...
    # Empty text
    response1 = client.post("/predict", json={"text": ""})
