...
async def get_analytics_summary(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        return AnalyticsSummaryResponse(
            ...
            avg_confidence=(
                round(float(row.avg_confidence), 4) if row.avg_confidence else 0.0
            ),
...
async def get_sentiment_distribution(
    ...
    try:
        ...
        stmt = select(
            Prediction.predicted_sentiment, func.count(Prediction.id).label("count")
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get sentiment distribution: {str(e)}"
        )
...
async def get_recent_predictions(
    request: Request,
    limit: int = Query(
        default=100, ge=1, le=1000, description="Number of recent predictions to return"
    ),
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get recent predictions: {str(e)}"
        )
